...
plot_sppr <- ggplot(sppr_df_ss, aes(x = value, y = Open)) +
  geom_point(aes(colour = Boundary_descriptor,shape=Season)) +
  scale_colour_manual(values=col_bound)+
  stat_poly_line() +
  stat_poly_eq() +
   theme_bw()+
...
plot_sppr <- ggplot(sppr_df_ss, aes(x = value, y = Open)) +
  geom_point(aes(colour = Boundary_descriptor,shape=Season)) +
  facet_wrap(~Boundary_descriptor) +
  scale_colour_manual(values=col_bound)+
  stat_poly_line() +
  stat_poly_eq() +
   theme_bw()+
...
#correlation between seed set yeild and species richness
cor(sppr_df_ss$value,sppr_df_ss$Open)
...
#correlation between seed set yeild and species richness
cor(sppr_df_ss$insect_counts,sppr_df_ss$Open)
...
m3<-lmer(Open ~ insect_counts + log(Flower_count)+(1|Season) +(1|Boundary_descriptor)+ (1|Site),data=sppr_df_ss)
summary(m3)
...
#by season
m3<-lmer(Open ~ insect_counts + log(Flower_count)+Season +(1|Boundary_descriptor)+ (1|Site),data=sppr_df_ss)
summary(m3)
qqnorm(residuals(m3))
qqline(residuals(m3))
plot(fitted(m3), residuals(m3))
...
#for paper:
Ordination.model2 <- rda(counts_wide.Hellinger ~ Boundary_descriptorf + `Flower count` + Season+ Condition(Site), data=meta_data_bior, scaling="species")
...
cap <- capscale(counts_wide ~ Boundary_descriptorf + `Flower count` +  Season + Condition(Site), data = meta_data_bior, distance = "bray")
