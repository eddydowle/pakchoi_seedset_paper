...
+ "B4BI_Pak-choi_seed_data_for_Heather.xlsx"  
...
The data is read in and some of the columns removed and renamed (after consulting with Brad). The garden data is also removed on Brad's instruction.

```{r}
pc <- read_excel("Excel_files_from_Brad/B4BI_Pak_choi_seed_data_for_Heather.xlsx") %>% 
...
+ The data is grouped by Year, Farm, Type_of_farm, Simplified_Boundary, Treatment, Rep.     
...
+ Then the data is grouped by Year, Farm, Type_of_farm, Simplified_Boundary, Treatment.  
...
+ If we didn't do the means with the two group_by steps when each pod would have the same weight in the mean.  

```{r}
pc_sum <- pc %>% 
  group_by(Year, Farm, Type_of_farm, Simplified_Boundary, Treatment, Rep) %>% 
  summarise(Mean_seeds_viable=mean(`Seed viable`)) %>%
  group_by(Year, Farm, Type_of_farm, Simplified_Boundary, Treatment) %>% 
