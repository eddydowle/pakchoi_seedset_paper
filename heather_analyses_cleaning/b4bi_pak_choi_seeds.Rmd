...
pc_sum %>% 
  mutate(Treatment=fct_recode(Treatment, "HP"="Hand pollinated"),
         Simplified_Boundary=fct_recode(Simplified_Boundary, 
                                        "Old native SNH"="Old native",
                                        "New native SNH"="New native",
                                        "On-farm bare fence"="Bare fence",
                                        "Off-farm bare fence"="Control farm"
                                        )) %>%
  ggplot(aes(x=Treatment, y=Mean_seeds_viable, color=Treatment:Type_of_farm)) + 
  geom_jitter(height=0, width=0.3, size=1) + 
  # geom_point(aes(y = mean_viable), size=2, color=1, data = pc_stats) +
  facet_grid(Year~Simplified_Boundary) + theme_bw(base_size=11) +
...
If the two outliers are removed from the data the modelling assumptions are better satisfied (results not shown).

The story is the same with the outliers in the model so they are not removed.
...
```{r}
mod.lm <- lmer(Mean_seeds_viable ~ Treatment*Simplified_Boundary + (1|Year/Farm), data=pc_mod_a)
...
using the FDR correction to account for multiple tests.
...
# Modelling (Updated so 'New Native' & Year one-to-one relationship)
...
The two outliers are not removed from the data. Removing them better satisfies
modelling assumptions, but does not change the story (results not shown).
