...
pc_sum %>% 
  ...
  geom_jitter(height=0, width=0.3, size=1) + 
  # geom_point(aes(y = mean_viable), size=2, color=1, data = pc_stats) +
  facet_grid(Year~Simplified_Boundary) + theme_bw(base_size=12) +
...
using the Šidák correction to account for multiple tests.
...
emm <- emmeans(mod.lm, ~Simplified_Boundary|Treatment, adjust="sidak") 
emm %>% as.data.frame() %>% 
  dplyr::select(-SE, -df) %>%
...
contrast(emm, method="pairwise", adjust="sidak", type = "response")
...
emm <- emmeans(mod.lm, ~Native_new_age|Treatment, adjust="sidak") 
emm %>% as.data.frame() %>% 
  dplyr::select(-SE, -df) %>%
  rename(`Predicted mean`=emmean) %>%
  kable(format="pandoc", digits=2)
